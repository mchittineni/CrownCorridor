...
def emit_latency_table(
    ...
    lines = [
        ...
        "\\texttt{results/run\\_manifest.json}.}",
        "\\label{tab:latency}",
        "\\centering",
        "\\small",
        "\\begin{tabular}{l l r}",
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = {
        ...
        "caveats": _caveats(n_negatives, unverified, total_unmapped),
    }
...
def _caveats(n_negatives: int, unverified: list[str], n_unmapped: int) -> list[str]:
    caveats = []
    if n_negatives == 0:
        caveats.append(
    ...
    if n_unmapped:
        caveats.append(
            f"{n_unmapped} findings were unmappable and are excluded from control-level "
            "scoring; this understates the affected tools."
        )
    caveats.append(
