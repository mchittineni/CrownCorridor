...
def emit_strictness_table(
    ...
    lines = [
        ...
        "$\\Delta$ is the control-to-any spread in percentage points, which "
        "measures how much of a tool's apparent detection is incidental. It is "
    ...
    for tool in sorted(results):
        ...
        control = results[tool]["control"].matrix.recall
        permissive = results[tool]["any"].matrix.recall
        spread = (permissive - control) * 100.0
        lines.append(f"{label} & {cells} & {spread:.2f} \\\\")
    lines += ["\\bottomrule", "\\end{tabular}", "\\end{table}", ""]
    return "\n".join(lines)
