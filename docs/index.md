# Crown Corridor — Documentation

> **Real Estate at Your Fingertips** — Next-generation property discovery portal for **Andhra Pradesh & Telangana**.

## Contents

| Document                                                                       | Description                                                                    |
| :----------------------------------------------------------------------------- | :----------------------------------------------------------------------------- |
| [index.html](index.html)                                                       | Interactive HTML documentation portal & navigation hub                         |
| [README.md](../README.md)                                                      | Repository overview, feature table, and local quickstart                       |
| [user-guide.md](user-guide.md)                                                 | A complete guide on how the portal, search, comparison, and audit reports work |
| [taxonomy.md](taxonomy.md)                                                     | IaCSecBench canonical control taxonomy and case-labelling scheme               |
| [benchmark_protocol.md](benchmark_protocol.md)                                 | Evaluation methodology, ground truth scoring, and reproducibility guidelines   |
| [framework/README.md](framework/README.md)                                     | Open-source IaCSecBench Security Evaluation Framework overview                 |
| [framework/architecture.md](framework/architecture.md)                         | IaCSecBench Framework architecture & pipeline workflow                         |
| [framework/benchmark-methodology.md](framework/benchmark-methodology.md)       | Standard evaluation methodology & mathematical metrics                         |
| [framework/metrics.md](framework/metrics.md)                                   | Benchmark evaluation metrics reference & target goals                          |
| [framework/adding-new-tests.md](framework/adding-new-tests.md)                 | Step-by-step guide for adding new benchmark scenarios                          |
| [framework/reproducible_experiments.md](framework/reproducible_experiments.md) | Reproducible experiment execution & telemetry verification guide               |
| [architecture.drawio](architecture.drawio)                                     | Official AWS Icons Draw.io XML diagram file                                    |
| [graph.png](graph.png) / [graph.svg](graph.svg)                                | Visual Terraform execution plan graph                                          |
| [api/](api/)                                                                   | JSDoc API reference for `application/app/portal.js` — generated by CI          |

---

## 🏛️ End-to-End Cloud Infrastructure Architecture

```mermaid
graph TD
    Client["📱 Mobile / Web Clients"] --> WAF["🛡️ AWS WAF (OWASP Top 10 & Rate Limits)"]
    WAF --> CDN["🌐 Amazon CloudFront CDN (TLS 1.3)"]

    CDN -->|Static Assets| S3["🪣 Amazon S3 Web UI (KMS, Versioning & Private OAC)"]
    CDN -->|API Queries| APIGW["⚡ Amazon API Gateway v2 (HTTP API)"]

    APIGW -->|JWT Auth| Cognito["🔐 Amazon Cognito User Pool (Advanced Security)"]
    APIGW -->|Proxy Pass| Fargate["🐳 AWS ECS Fargate Cluster"]

    subgraph Compute ["ECS Fargate Private Compute"]
        FastAPI["🚀 FastAPI Microservice (application/api/main.py)"]
        Typesense["🔍 Typesense Search Engine (Sub-10ms)"]
        EFS["💾 Amazon EFS Persistent Storage"]
        FastAPI <--> Typesense
        Typesense <--> EFS
    end

    Fargate <--> DB["🐘 Amazon RDS PostGIS (Private Multi-AZ)"]

    subgraph Security ["Security, Auditing & Event Layer"]
        CloudTrail["📜 AWS CloudTrail (API Audit Logs)"]
        GuardDuty["🛡️ AWS GuardDuty (Threat Detection)"]
        SecurityHub["📊 AWS Security Hub (CIS Benchmarks)"]
        SecretsMgr["🔑 AWS Secrets Manager (KMS Encrypted)"]
        EventBridge["⏱️ Amazon EventBridge (Weekly Cron)"]
        SNS["🔔 Amazon SNS Alerting Topic"]

        EventBridge -->|Scheduled ETL| Fargate
        EventBridge -->|Notifications| SNS
        GuardDuty -->|Security Alerts| SNS
        SecurityHub -->|Compliance Alerts| SNS
    end
```

---

## 🔬 IaCSecBench Benchmark Dataset Architecture

```mermaid
graph TD
    Bench["🔬 IaCSecBench Framework"] --> Internal["🔒 Controlled Benchmark (Internal)"]
    Bench --> External["🌐 Validation Datasets (External)"]
    Bench --> Ops["⚡ Operational Observation"]

    Internal --> IntCases["Labelled vulnerable/compliant pairs<br/>(12 Domains: IAM, Net, Storage, etc.)"]

    External --> TFReg["📦 Terraform Registry<br/>(declares 50; 0 present)"]
    External --> SF["🛡️ SecureFlag IaC<br/>(declares 75; 0 present)"]
    External --> CIS["📋 CIS AWS Benchmarks<br/>(declares 50; 4 present)"]

    Ops --> PRs["🚀 61 CI/CD Pull Requests"]
```

---

## Architecture Overview

```
Browser & Web Portal (application/app/)
  └── application/app/index.html   Dashboard entry point
        ├── styles.css            Glassmorphism dark & light theme design system
        └── portal.js             All portal logic:
              ├── Global Search Real-time API query with debounced autocomplete & local fallback
              ├── Compare Tool  Side-by-side spec comparison modal for up to 3 properties
              ├── SRO Ticker    Live registration feed with pause/resume and speed control
              ├── Map Explorer  Leaflet map + GPS "Locate Me" distance calculation
              ├── Sale History  State-modular registry audit with CAGR analytics & POIs
              ├── Audit Report  One-click printable PDF valuation report generator
              ├── Calculators   Stamp duty (AP 7.5%, TS 6.0%) and guidance-value estimator
              └── API Console   JSON query sandbox and webhook configuration

Fast-Read API & Search Service (application/api/)
  ├── application/api/main.py     Async FastAPI endpoints (/health, /api/v1/search, /api/v1/properties)
  └── application/api/search.py   Typesense connection pool manager & multi-field query builder

Terraform Infrastructure as Code (infrastructure/)
  ├── providers.tf         Terraform >= 1.15.0 & AWS provider ~> 6.56.0 constraint
  ├── main.tf              Master orchestrator connecting 10 child modules
  ├── variables.tf / outputs.tf Root inputs and exported endpoint URLs
  ├── tests/               Centralized Native .tftest.hcl test directory (11 suites, 26 assertions)
  └── modules/             Reusable child modules (vpc, security, waf, cdn, auth, api_gateway, compute, database, secrets_ssm, events_alerting)

IaCSecBench Security Framework & Evaluation Protocol (security_framework/ & evaluation/)
  ├── evaluation/metrics.py       Evaluation metrics calculation engine (Acc, Prec, Rec, F1, Latency)
  ├── evaluation/run_baselines.py The measurement harness (executes the scanners)
  ├── evaluation/analyze.py       Statistics, LaTeX tables, measured leaderboard CSV
  ├── evaluation/score.py         DEPRECATED: fabricates metrics from assumed rates; gated
  ├── security_framework/engine/  Engine scanners and comparative benchmark evaluator
  └── security_framework/policies/ OPA Rego policy definitions

Benchmark Datasets, Cases & Leaderboard (benchmark/ & leaderboard/)
  ├── benchmark/internal/         Internal self-contained test case catalog
  ├── benchmark/external/         External collections. The manifests DECLARE 175 cases;
  │                              far fewer configurations exist on disk. See
  │                              results/corpus_report.json for declared vs present.

  ├── benchmark/golden_results/  Golden reference JSON outputs for Checkov, tfsec, Trivy, OPA, IaCSecBench
  └── leaderboard/results.csv    Measured leaderboard, generated by evaluation/analyze.py

Experiments & Results (experiments/ & results/)
  ├── experiments/run_all.sh      One-command reproducible experiment suite
  ├── experiments/generate_charts.py Benchmark performance chart generator
  └── results/                    Measured outputs only: run_manifest.json, evaluation.json,
                                  corpus_report.json, raw/, tables/. See results/README.md.
```
