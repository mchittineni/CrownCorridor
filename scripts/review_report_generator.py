...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate a combined review report from repo analysis tools.")
    ...
    parser.add_argument("--framework", choices=["soc2", "pci-dss", "hipaa", "gdpr"], default="soc2", help="Compliance framework to evaluate.")
...
def main():
    ...
    quality_cmd = [sys.executable, "scripts/code_quality_checker.py", target, "--language", "python", "--json"]
    pr_cmd = [sys.executable, "scripts/pr_analyzer.py", target, "--json"]
    if args.base and args.head:
        pr_cmd.extend(["--base", args.base, "--head", args.head])
    compliance_cmd = [sys.executable, "scripts/compliance_checker.py", target, "--framework", args.framework, "--json"]

    for name, cmd in [("security", security_cmd), ("quality", quality_cmd), ("pr", pr_cmd), ("compliance", compliance_cmd)]:
